...
fn exception_instance(ty: Option<Rc<TypeObject>>, value: Object) -> Object {
    ...
    // CPython `PyErr_SetObject` / `_PyErr_NormalizeException`: a *tuple* value
    // is the exception constructor's argument list, not a message. Run the
    // type's real `__new__`/`__init__` (through the interpreter) so a
    // C-extension exception with a custom constructor keeps the attributes its
    // `__str__` reads. numpy raises `_UFuncBinaryResolutionError` via
    // `PyErr_SetObject(exc, (ufunc, (dt0, dt1)))`, and only
    // `UFuncTypeError.__init__` stores the `self.ufunc`/`self.dtypes` that its
    // `__str__` dereferences; the message-stringifying fallback below collapses
    // the tuple into a single `args[0]`, so `str(exc)` then raised
    // `AttributeError: '_UFuncBinaryResolutionError' object has no attribute
    // 'ufunc'`. Construction runs before we publish the slot below, so it may
    // freely use the (still-old) pending cell.
    if matches!(&value, Object::Tuple(_)) {
        if let Some(Ok(inst @ Object::Instance(_))) = crate::interp::with_interp_mut(|interp| {
            interp.construct_exception_from_capi(class.clone(), value.clone())
        }) {
            return inst;
        }
    }
...
pub fn set_pending(ty: Option<Rc<TypeObject>>, value: Object) {
    ...
    // Diagnostic: print a Rust backtrace for *any* exception whose type name
    // contains the `WEAVEPY_TRACE_RAISE_ANY` needle. Unlike the RuntimeError
    // trace above this covers ValueError/TypeError/etc., which is how we locate
    // the C-API primitive that raises inside a compiled Cython call (e.g. a
    // spurious ValueError swallowed by `guess_datetime_format`'s `except`).
    if let Some(needle) = std::env::var_os("WEAVEPY_TRACE_RAISE_ANY") {
        let name = match &inst {
            Object::Instance(i) => i.cls().name.clone(),
            other => other.type_name_owned(),
        };
        let needle = needle.to_string_lossy();
        if needle.is_empty() || name.contains(needle.as_ref()) {
            eprintln!(
...
pub fn to_runtime_error(p: PendingError) -> RuntimeError {
    ...
    // the pending error carried no resolvable type. (Tuple/constructor
    // normalisation already happened eagerly in `exception_instance` when the
    // error was set, so `p.value` is an instance or a plain message here.)
    let class =
...
pub fn set_pending_from_runtime(err: RuntimeError) {
    match err {
        RuntimeError::PyException(pe) => {
            // Preserve the exception *instance* verbatim rather than reducing
            // it to `(class, message)`. A real exception instance may carry
            // attributes that only its constructor set and that its `__str__`
            // later dereferences — numpy's `_UFuncBinaryResolutionError`
            // stores `self.ufunc`/`self.dtypes`. The old
            // `set_pending(cls, from_str(pe.message()))` dropped them, so a
            // downstream `str(exc)` raised `AttributeError: … has no attribute
            // 'ufunc'`. Passing the instance as the pending *value* lets
            // `exception_instance` take its "already an instance" fast path.
            match pe.instance {
                inst @ Object::Instance(_) => {
                    let cls = match &inst {
                        Object::Instance(i) => Some(i.cls()),
                        _ => None,
                    };
                    set_pending(cls, inst);
                }
                other => set_pending(None, Object::from_str(message_for(&other))),
            }
        }
...
pub fn init_static_exceptions() {
    ...
    *done = true;
    ...
    unsafe {
        ...
        publish(&raw mut PyExc_ReferenceError, bt.reference_error.clone());
        ...
        publish(&raw mut PyExc_SystemError, bt.system_error.clone());
        publish(&raw mut PyExc_SystemExit, bt.system_exit.clone());
        publish(&raw mut PyExc_TabError, bt.tab_error.clone());
        ...
        publish(
            ...
            bt.unicode_decode_error.clone(),
        );
        publish(
            &raw mut PyExc_UnicodeEncodeError,
            bt.unicode_encode_error.clone(),
        );
        publish(&raw mut PyExc_UnicodeError, bt.unicode_error.clone());
        publish(
            &raw mut PyExc_UnicodeTranslateError,
            bt.unicode_translate_error.clone(),
        );
...
#[no_mangle]
pub unsafe extern "C" fn PyErr_Restore(ty: *mut PyObject, value: *mut PyObject, tb: *mut PyObject) {
    ...
    set_pending(cls, v);
    // `PyErr_Restore` *steals* all three references (CPython consumes them
    // when the error is eventually cleared). `set_pending` installed its own
    // owned reference in the slot; releasing the caller's here keeps the
    // fetch/restore round-trip balanced. Leaking them made the fetched
    // exception instance immortal, and its `__traceback__` pinned the
    // raising frame's *arguments* (+1 per Cython `PyErr_Fetch`/`PyErr_Restore`
    // round-trip: pandas' `map_infer_mask` leaked `self` of every `.str`
    // accessor call, which pinned Series/DataFrames repr'd once forever).
    unsafe {
        crate::object::Py_DecRef(ty);
        if !value.is_null() {
            crate::object::Py_DecRef(value);
        }
        if !tb.is_null() {
            crate::object::Py_DecRef(tb);
        }
    }
}
...
unsafe fn warn_via_warnings(
    ...
    let module =
        unsafe { crate::module::PyImport_ImportModule(b"warnings\0".as_ptr() as *const c_char) };
    if module.is_null() {
...
pub unsafe extern "C" fn PyErr_NewException(
    ...
    let bare = qualified
        ...
        .unwrap_or(&qualified)
        .to_owned();
    let new_ty = match TypeObject::new_user(&bare, vec![base_ty], DictData::default()) {
        Ok(t) => t,
        Err(_) => {
