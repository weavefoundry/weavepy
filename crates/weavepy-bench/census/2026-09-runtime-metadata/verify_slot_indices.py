"""Check slot order mutations and native indexed access coverage."""
...
def main():
    ...
    for name, binary, flags, jit in (
        ...
        result = subprocess.run(
            [binary, *flags, str(ROOT / "tests/regrtest/test_indexed_slot_access.py")],
    ...
    steady = {}
    kernels = runpy.run_path(str(HERE / "jit_slot_probes.py"))["KERNELS"]
    ...
    assert "total_value" in runs["new"]["compiled"]
