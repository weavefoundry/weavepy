...
/// guard is returned by [`load`] and held for the whole test.
...
fn make(interp: &mut Interpreter, class: &str, args: &[i64]) -> Object {
    ...
    let cls = lookup(&module, class).unwrap_or_else(|| panic!("datetime module missing `{class}`"));
...
fn stockdatetime_read_datetime() {
    ...
    let dt = make(&mut interp, "datetime", &[2021, 3, 14, 9, 30, 15, 123_456]);
    let got = int_tuple(call(&mut interp, &module, "read_datetime", &[dt.clone()]));
    assert_eq!(got, vec![2021, 3, 14, 9, 30, 15, 123_456, 0]);
...
fn stockdatetime_read_time() {
    ...
    let t = make(&mut interp, "time", &[9, 30, 15, 123_456]);
    let got = int_tuple(call(&mut interp, &module, "read_time", &[t]));
    assert_eq!(got, vec![9, 30, 15, 123_456, 0]);
}
...
fn stockdatetime_read_delta() {
    ...
    let td = make(&mut interp, "timedelta", &[5, 3600, 250_000]);
    let got = int_tuple(call(&mut interp, &module, "read_delta", &[td]));
    assert_eq!(got, vec![5, 3600, 250_000]);
}
