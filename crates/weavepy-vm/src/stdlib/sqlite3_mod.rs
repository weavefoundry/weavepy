...
pub fn build(_cache: &ModuleCache) -> Rc<PyModule> {
    let dict = Rc::new(RefCell::new(DictData::new()));
    {
        ...
        d.insert(
            DictKey(Object::from_static("threadsafety")),
            Object::Int(1),
        );
        d.insert(
    ...
    let lastrowid: Rc<Cell<i64>> = Rc::new(Cell::new(0));
    {
        ...
        d.insert(
            ...
            builtin("execute", move |args: &[Object]| {
                run_execute(
                    &conn_e, args, &rows_e, &desc_e, &rowcount_e, &lastrowid_e,
                )?;
                Ok(Object::None)
            }),
        );
        ...
        d.insert(
            ...
            builtin("executemany", move |args: &[Object]| {
                run_executemany(
                    &conn_em, args, &rows_em, &desc_em, &rowcount_em, &lastrowid_em,
                )?;
                Ok(Object::None)
            }),
        );
...
fn object_to_sql(o: &Object) -> Result<SqlValue, RuntimeError> {
    Ok(match o {
        ...
        Object::Bool(b) => SqlValue::Integer(if *b { 1 } else { 0 }),
