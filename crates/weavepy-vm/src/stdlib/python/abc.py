...
# WeavePy has no `_abc` C accelerator, so `ABCMeta` is implemented in
# pure Python (mirroring `_py_abc`, which stays a separate module that
# `test_abc` imports to exercise the reference implementation). It is
# defined *here* rather than imported so that its frames' module is
# `'abc'`: `typing._allow_reckless_class_checks()` walks the frame stack
# and only skips the runtime-protocol restrictions when the caller
# module is `'abc'` or `'functools'` (e.g. `isinstance(x, Traversable)`
# reaches `_ProtocolMeta.__subclasscheck__` *via*
# `ABCMeta.__instancecheck__`, which CPython sees as module `'abc'`
# because that's where the delegating method lives).
