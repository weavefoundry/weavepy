"""Capture a warm-cache startup live-allocation trace; retain its cold-cache predecessor."""
import gzip,hashlib,json,os,selectors,subprocess
from pathlib import Path
root=Path('target/binder-flags-startup-live-allocations-warm');assert not root.exists();root.mkdir()
...
env.update(WEAVEPY_JIT='1',WEAVEPY_STDLIB_CACHE=str(Path('target/performance-stdlib-cache').resolve()),WEAVEPY_FROZEN_CACHE=str(Path('target/binder-flags-startup-live-allocations/frozen').resolve()),MallocStackLogging='1',MallocStackLoggingDirectory=str(logs.resolve()))
...
cache=Path(env['WEAVEPY_FROZEN_CACHE'])
def cache_snapshot():
    return {str(p.relative_to(cache)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(cache.rglob('*')) if p.is_file()}
report['frozen_cache_before']=cache_snapshot()
report['cold_predecessor']='target/binder-flags-startup-live-allocations/report.json'
...
try:
    ...
    report['frozen_cache_after']=cache_snapshot()
    report['frozen_cache_unchanged']=report['frozen_cache_before']==report['frozen_cache_after']
