...
pub unsafe extern "C" fn PyLong_AsInt(obj: *mut PyObject) -> c_int {
    ...
    // `v` is the raw 64-bit value (`PyLong_AsLong` returns `i64` on every
    // platform, unlike C's `long`), so bounds-check in `i64`.
    if v < i64::from(c_int::MIN) || v > i64::from(c_int::MAX) {
    ...
    let cname = match std::ffi::CString::new(abs_name.clone()) {
        ...
        Err(_) => {
            crate::errors::set_value_error(
                "PyImport_ImportModuleLevelObject: embedded NUL in name",
            );
            return ptr::null_mut();
        }
    };
            ...
            if trace_imp {
                ...
                if !has_attr {
                    if let Object::Module(m) = unsafe { crate::object::clone_object(module) } {
                        let d = m.dict.borrow();
                        let n = d.len();
                        let has_via_iter = d.iter().any(
                            |(k, _)| matches!(&k.0, Object::Str(s) if s.as_ref() == sub.as_str()),
                        );
...
/// Drop every `_PyType_Lookup` memo. `PyType_Modified` calls this — the
/// entries hold *borrowed* pointers whose referents stay alive via the
/// type dicts, so flushing only costs re-resolution on the next lookup.
pub(crate) fn flush_type_lookup_cache() {
    type_lookup_cache().lock().unwrap().clear();
}
