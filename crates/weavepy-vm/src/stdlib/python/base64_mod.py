...
# This is a verbatim port of CPython 3.13's Lib/base64.py. It is pure
# Python over `binascii` + `struct` + `re`; WeavePy's frozen copy keeps it
# byte-for-byte so the a85/b85/z85 encoders and the altchars/validate edges
# match CPython exactly. The previous Rust `base64` module covered only the
# RFC 3548 subset and silently ignored `altchars`/`validate`.
