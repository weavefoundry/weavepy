...
/* WeavePy: vendored verbatim from CPython 3.14 Modules/_testbuffer.c
   (RFC 0066 WS1, refreshed by RFC 0077). Deviations from upstream are
   marked with `WeavePy` comments: header shims plus the `get_nmemb`
   member-count probe. */

#include "Python.h"
#include <assert.h> /* WeavePy: CPython's Python.h includes this transitively */
...
{
    /* WeavePy deviation: upstream reads the private
       `PyStructObject.s_len` field off the C struct layout (the
       issue #12740 hack above). WeavePy's `struct.Struct` is not
       byte-compatible with `_struct.c`, so derive the member count
       portably as `len(s.unpack(bytes(s.size)))`. */
    PyObject *size_obj, *zeros, *res;
    Py_ssize_t size, nmemb;
    size_obj = PyObject_GetAttrString(s, "size");
    if (size_obj == NULL)
        return -1;
    ...
    if (size < 0)
        return -1;
    zeros = PyBytes_FromStringAndSize(NULL, size);
    if (zeros == NULL)
        return -1;
    memset(PyBytes_AS_STRING(zeros), 0, (size_t)size);
    res = PyObject_CallMethod(s, "unpack", "O", zeros);
    Py_DECREF(zeros);
    if (res == NULL)
        return -1;
    nmemb = PySequence_Length(res);
    Py_DECREF(res);
    return nmemb;
}
