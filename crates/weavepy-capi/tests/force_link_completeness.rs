...
fn every_no_mangle_export_survives_dead_strip() {
    // A PE executable exports nothing unless each symbol is explicitly
    // `dllexport`ed — there is no `--export-dynamic` equivalent — and the
    // Windows loader resolves an extension's imports through import
    // libraries, not the host executable's export table. The dead-strip
    // hazard this test guards against is an ELF/Mach-O dynamic-linker
    // behaviour, and checking the (empty) PE export table would report
    // every symbol as missing.
    // (Runtime check, not `#[cfg]`, so the helpers above stay compiled on
    // Windows and don't trip `-D warnings` dead-code errors.)
    if cfg!(windows) {
        eprintln!("skipping: PE executables have no dlopen-visible export table");
        return;
    }
