...
pub fn b_dumps(args: &[Object]) -> Result<Object, RuntimeError> {
    let value = args.first().ok_or_else(|| type_error("dumps requires a value"))?;
...
fn b_load(args: &[Object]) -> Result<Object, RuntimeError> {
    let f = args.first().ok_or_else(|| type_error("load() requires a file"))?;
    let bytes = match f {
        ...
        match tag {
            ...
            TYPE_FLOAT => {
                ...
                let s = std::str::from_utf8(&bytes)
                    .map_err(|_| value_error("bad marshal float"))?;
            ...
            TYPE_LONG => {
                ...
                let big = BigInt::from_slice(
                    if signed_count < 0 { Sign::Minus } else { Sign::Plus },
                    &digits,
                );
            ...
            TYPE_SHORT_ASCII | TYPE_SHORT_ASCII_INTERNED => {
                ...
                let bytes = self.read_n_bytes(len)?;
                let s = String::from_utf8(bytes)
                    .map_err(|_| value_error("bad marshal short ascii"))?;
                Ok(Object::from_str(s))
            }
            TYPE_TUPLE => {
                let len = self.read_int()? as usize;
            ...
            other => Err(value_error(format!(
                "marshal: unknown type tag {other:?}"
            ))),
        }
    }
}
