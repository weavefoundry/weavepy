...
// Process-global (RFC 0047, wave 5): a module imported on the main thread
// serves `PyModule_GetState` / `PyState_FindModule` calls from any
// `threading.Thread` — pandas' ujson encoder does both at run time — so a
// per-thread registry would hand workers NULL state and re-run the
// registration.
static MODULE_STATE: std::sync::Mutex<Option<std::collections::HashMap<usize, Box<[u8]>>>> =
...
static FIND_MODULE: std::sync::Mutex<Option<std::collections::HashMap<usize, usize>>> =
    std::sync::Mutex::new(None);
...
pub extern "C" fn PyModule_GetState(module: *mut PyObject) -> *mut c_void {
    ...
    MODULE_STATE
        ...
        .and_then(|mut g| {
            g.as_mut()
                .and_then(|m| m.get_mut(&key).map(|buf| buf.as_mut_ptr() as *mut c_void))
        })
        .unwrap_or(ptr::null_mut())
}
...
pub extern "C" fn PyState_FindModule(def: *mut c_void) -> *mut PyObject {
    ...
    FIND_MODULE
        ...
        .and_then(|g| {
            g.as_ref()
                .and_then(|m| m.get(&(def as usize)).map(|&p| p as *mut PyObject))
        })
        .unwrap_or(ptr::null_mut())
}
...
pub unsafe extern "C" fn PyList_SetSlice(
    ...
    // Keep a faithful list mirror's inline `ob_item` coherent with the
    // spliced prefix `Rc` so a stock `PyList_GET_ITEM` macro reads the new
    // contents/length (RFC 0047, wave 5; see `PyObject_SetItem`).
    unsafe { crate::mirror::sync_list_ob_item(list) };
    0
}
...
pub unsafe extern "C" fn PyException_GetTraceback(exc: *mut PyObject) -> *mut PyObject {
    ...
    let tb = unsafe { crate::abstract_::PyObject_GetAttrString(exc, c"__traceback__".as_ptr()) };
    if tb.is_null() {
...
pub unsafe extern "C" fn PyStaticMethod_New(callable: *mut PyObject) -> *mut PyObject {
    ...
    crate::object::into_owned(Object::StaticMethod(
        weavepy_vm::object::MethodWrapper::new(func),
    ))
}
