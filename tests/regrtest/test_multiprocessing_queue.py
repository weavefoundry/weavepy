...
def main():
    ...
    # Two producers feed 20 items total; two consumers race to drain
    # them. We can't pin each consumer to a fixed item count because
    # the OS scheduler decides who gets which item, and a "lost the
    # race" consumer would time out on Empty. Instead, both consumers
    # share a counter and stop once the expected total is drained.
    q = multiprocessing.Queue()
    received = []
    lock = threading.Lock()
    target_total = 20
    ...
    def consumer():
        while True:
            ...
            try:
                ...
                # Empty under load: re-check the shared counter; the
                # other consumer may have just finished.
                continue
            with lock:
                received.append(v)
