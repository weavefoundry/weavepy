...
WeavePy: vendored from CPython 3.14 `Lib/traceback.py` verbatim, except
one nested-same-quote f-string (PEP 701) rewritten for the lexer and the
memoized caret-anchor helpers below.
"""
...
class StackSummary(list):
        ...
        if frame_summary._dedented_lines and frame_summary._dedented_lines.strip():
            if (
                ...
                with suppress(Exception):
                    anchors = _extract_caret_anchors_cached(segment)
    ...
    def _should_show_carets(self, start_offset, end_offset, all_lines, anchors):
        # WeavePy: memoized — the decision is a pure function of its
        # inputs and the `ast.parse` probe dominates repeated renders
        # (e.g. a 1000-deep `__context__` chain re-rendering one line).
        key = (start_offset, end_offset, tuple(all_lines), anchors)
        cached = _SHOW_CARETS_CACHE.get(key)
        if cached is not None:
            return cached
        result = self._should_show_carets_uncached(
            start_offset, end_offset, all_lines, anchors)
        if len(_SHOW_CARETS_CACHE) >= 512:
            _SHOW_CARETS_CACHE.clear()
        _SHOW_CARETS_CACHE[key] = result
        return result

    def _should_show_carets_uncached(self, start_offset, end_offset, all_lines, anchors):
...
# WeavePy: anchor extraction is a pure function of the segment text and
# costs an `ast.parse`; memoize so repeated renders of the same line
# (recursive tracebacks, long context chains) stay cheap.
_ANCHORS_CACHE = {}
_SHOW_CARETS_CACHE = {}
_ANCHORS_MISS = object()

def _extract_caret_anchors_cached(segment):
    cached = _ANCHORS_CACHE.get(segment, _ANCHORS_MISS)
    if cached is not _ANCHORS_MISS:
        return cached
    result = _extract_caret_anchors_from_line_segment(segment)
    if len(_ANCHORS_CACHE) >= 512:
        _ANCHORS_CACHE.clear()
    _ANCHORS_CACHE[segment] = result
    return result
