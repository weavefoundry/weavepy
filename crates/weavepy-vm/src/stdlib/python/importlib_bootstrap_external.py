...
# runtime and `importlib/__init__.py` (or interpreter startup) wires the
# `_bootstrap` global. WeavePy imports it as an ordinary frozen source under
# both the `importlib._bootstrap_external` and `_frozen_importlib_external`
# names, so wire ourselves to whichever bootstrap instance is live.
def _weave_self_wire():
    import sys as s
    b = (s.modules.get('importlib._bootstrap')
         or s.modules.get('_frozen_importlib'))
    if b is None:
        import _frozen_importlib as b
    _set_bootstrap_module(b)
    me = s.modules.get(__name__)
    if me is not None and getattr(b, '_bootstrap_external', None) is None:
        b._bootstrap_external = me


_weave_self_wire()
del _weave_self_wire
