...
//!
//! On macOS the `weavepy` binary is linked with `weavepy.order`
//! (`-order_file`, regenerated by `scripts/gen-order-file.sh`): the
//! functions start-up and common workloads run, laid out in first-use
//! order, so a process maps a few contiguous megabytes of code instead
//! of touching pages spread over the whole text segment. Symbols the
//! file names that the build does not define are ignored, so a stale
//! file costs only the layout benefit. `WEAVEPY_NO_ORDER_FILE` opts out.
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=WEAVEPY_NO_ORDER_FILE");
