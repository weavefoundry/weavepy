...
"""Generate WeavePy's UCD 16.0.0 tables by probing host CPython 3.14.

RFC 0050 WS4 (UCD 16.0.0 for 3.14 parity in RFC 0077). CPython 3.14's
`unicodedata` *is* UCD 16.0.0, so probing it
...
assert sys.version_info[:2] == (3, 14), "must run under CPython 3.14"
...
ALGORITHMIC_PREFIXES = (
    ...
    "NUSHU CHARACTER-",
    "EGYPTIAN HIEROGLYPH-",  # algorithmic since UCD 16.0.0 (U+13460..)
)
...
def probe_aliases_and_sequences():
    ...
    seqs = bytearray()
    path = os.path.join(DATA_DIR, "NamedSequences-%s.txt" % UCD_VERSION)
    for line in open(path, encoding="utf-8"):
        line = line.strip()
        if not line or line.startswith("#"):
            continue
...
def probe_lookup_only_ranges():
    ...
    accept but `name()` does not generate — Tangut, in UCD 15.1.0 and
    16.0.0."""
    runs = []
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "meta.rs"), "w") as f:
        f.write(
            "// Generated by tools/gen_ucd_tables.py from CPython 3.14\n"
            "// (UCD 16.0.0). Do not edit.\n\n"
        )
