...
### The pandas test suite under WeavePy

The end-state acceptance run: the **entire pandas 2.2 test suite** (the
binary wheel's bundled `pandas/tests`, 958 test files) executed under the
WeavePy release binary with real numpy, using pandas' own default mark
filter (`-m 'not slow and not network and not db'` — what `pd.test()`
runs):

- **173,720 passed**, 16,051 skipped, 942 xfailed, 84 xpassed across
  **956 of 958 files clean** (2 files collect no tests under the mark
  filter, matching CPython).
- The only 2 files with failures are **environment-identical under stock
  CPython 3.13** on this machine, not WeavePy defects:
  - `tests/io/test_clipboard.py` — 321 fixture errors (the `qtpy`/`qapp`
    clipboard fixture; PyQt is not installed). CPython: same 321.
  - `tests/arrays/test_array.py` — 1 failure
    (`test_array_inference[data7-expected7]`, the `pytz` CET LMT-offset
    incompatibility). CPython: same 1.
- Long-tail work that got the sweep here included: full `__index__`
  protocol on `list.__getitem__`/`__setitem__` reached through
  `super()` on list subclasses (pandas `FrozenList._reorder_ilevels` with
  numpy integer scalars — 55 failures in `test_stack_unstack.py` alone);
  `typing.NewType` as a class (hypothesis detects NewType aliases with
  `isinstance`); frame-free native `__call__` for
  `operator.attrgetter`/`itemgetter`/`methodcaller` (pandas'
  `find_stack_level()` warning attribution counts stack frames);
  `pip.__version__` on the frozen minipip (`pd.show_versions`); and
  pytest-shim fidelity (capsys teardown + `capsys.disabled()` context
  manager, `py.path.local`-style `tmpdir`, fixture-before-mark
  `callspec.id` ordering, xunit `setup_method`/`teardown_method`).
