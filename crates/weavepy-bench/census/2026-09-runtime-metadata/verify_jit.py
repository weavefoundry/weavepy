"""Check enumeration parity and confirm that the new paths compile."""
...
def main():
    ...
    for name, binary, flags, jit in (
        ...
        result = subprocess.run([binary, *flags, str(ROOT / "tests/regrtest/test_jit_enumerate.py")],
        ...
        print(name, result.returncode, "compiled targets:",
              sorted(set(compiled) & {"enum_checksum", "sum_indices", "last_item", "local_indices"}), flush=True)
    steady = {}
    kernels = runpy.run_path(str(Path(__file__).with_name("jit_probes.py")))["KERNELS"]
    for name, source in kernels.items():
        ...
        steady[name] = {
            ...
            "compiled": 'jit compile "checksum"' in result.stderr,
            "deopts": len(re.findall(r'jit deopt "checksum"', result.stderr)),
    ...
    report = {"runs": runs, "binaries": {
        ...
        for name, binary in (("base", args.base), ("new", args.new))}, "steady_paths": steady}
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(report, indent=2) + "\n")
    assert all(run["returncode"] == 0 and run["stdout"] == "ok\n" for run in runs.values())
    assert {"enum_checksum", "sum_indices", "last_item", "local_indices"} <= set(runs["new"]["compiled"])
