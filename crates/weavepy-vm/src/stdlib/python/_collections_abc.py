...
# WeavePy: ``range`` does not accept arbitrary-precision (big int) bounds
# yet, so ``range(1 << 1000)`` raises. Every ``range`` shares one iterator
# type here, so reuse it rather than probing an out-of-range bound.
try:
...
# WeavePy: frame ``f_locals`` is a plain ``dict`` rather than a distinct
# ``FrameLocalsProxy`` type (PEP 667), so fall back to that. The registry
# below registers it with ``Mapping``; ``dict`` is already a Mapping, so
# the duplicate registration is harmless.
