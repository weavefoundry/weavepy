...
def _strip_step_frame(exc):
    # WeavePy runs CPython's pure-Python ``asyncio.Task`` — there is no
    # ``_asyncio`` C accelerator. The C ``Task`` drives the coroutine from
    # C, so an exception propagating out of the coroutine carries *no*
    # Python frame for the step itself. The pure-Python ``Task`` calls
    # ``coro.send()``/``throw()`` from ``__step_run_and_handle_result``,
    # which therefore sits at the head of the captured traceback. Drop that
    # one frame so stored task exceptions match the default (C-accelerated)
    # CPython traceback shape the stdlib asserts — e.g.
    # ``test_contextlib_async.test_exit_exception_traceback``.
...
class Task(futures._PyFuture):  # Inherit Python Task implementation
    ...
    def __step_run_and_handle_result(self, exc):
        ...
        try:
            ...
            super().set_exception(_strip_step_frame(exc))
            raise
