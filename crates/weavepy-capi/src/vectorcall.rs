...
pub unsafe extern "C" fn PyObject_VectorcallMethod(
    ...
    let dbg = std::env::var_os("WEAVEPY_VCM_DBG").is_some();
    if dbg {
        let mname = match unsafe { crate::object::clone_object(name) } {
            Object::Str(s) => s.to_string(),
            _ => "<non-str>".to_string(),
        ...
        for i in 0..nargs {
            ...
            use std::fmt::Write;
            let _ = write!(raw, " a{i}={p:p}");
        }
        eprintln!(
            "[VCM] pre-collect name={mname} nargs={nargs} kwnull={} raw=[{raw}]",
            kwnames.is_null()
        );
    }
    let positional = unsafe { collect_positional_after(args, nargs) };
    if dbg {
        let kinds: Vec<&str> = positional.iter().map(obj_kind).collect();
        eprintln!("[VCM] pre-call collected argkinds={kinds:?}");
    }
    let arg_tuple = crate::object::into_owned(Object::new_tuple(positional));
    ...
    if dbg {
        eprintln!("[VCM] post-call result_null={}", result.is_null());
    }
    unsafe { crate::object::Py_DecRef(arg_tuple) };
    unsafe { crate::object::Py_DecRef(kw_dict) };
...
/// Debug-only: the VM-variant name of a cloned [`Object`].
fn obj_kind(o: &Object) -> &'static str {
    match o {
        Object::None => "None",
        Object::Bool(_) => "Bool",
        Object::Int(_) => "Int",
...
unsafe fn decode_vectorcall(
    ...
    let kwargs = if kwnames.is_null() {
        weavepy_vm::object::DictData::default()
    } else {
