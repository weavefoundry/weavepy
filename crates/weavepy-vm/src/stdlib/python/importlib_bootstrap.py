...
def _setup(sys_module, _imp_module):
    ...
    # WeavePy: snapshot the items — attribute probes on WeavePy's lazy
    # native modules can materialize submodules into sys.modules while
    # we iterate (CPython runs this before any lazy module exists).
    ...
    for name, module in list(sys.modules.items()):
        if isinstance(module, module_type):
            ...
            if loader is FrozenImporter:
                # WeavePy: only genuinely-frozen startup modules carry the
                # `__origname__` breadcrumb CPython's C importer leaves;
                # modules pre-imported by WeavePy's native importer already
                # have complete attrs (including `__file__`), which
                # `_fix_up_module`'s asserts reject.
                if getattr(module, '__origname__', None) is not None:
                    loader._fix_up_module(module)
                elif getattr(spec, 'loader_state', None) is None:
                    # Synthesize the loader_state `_fix_up_module` would
                    # have built from the breadcrumb, so a *re-run* of
                    # `_setup` — CPython's verbatim `_bootstrap.py`
                    # re-imported from disk by test_importlib's
                    # source-variant machinery — finds the state complete
                    # and skips its asserts, exactly as on CPython where
                    # the C importer always leaves the breadcrumb.
                    ispkg = hasattr(module, '__path__')
                    try:
                        filename, _pkgdir = FrozenImporter._resolve_filename(
                            spec.name, spec.name, ispkg)
                    except Exception:
                        filename = None
                    spec.loader_state = type(sys.implementation)(
                        filename=filename, origname=spec.name)
...
# ---------------------------------------------------------------------------
# WeavePy adaptation (RFC 0068 WS4). In CPython this file is frozen into the
# runtime and the interpreter calls `_setup(sys, _imp)` during startup. In
# WeavePy the live import system is native and this module is imported as an
# ordinary frozen source (under both the `importlib._bootstrap` and
# `_frozen_importlib` names), so bootstrap ourselves on import.
import sys as _weave_sys
import _imp as _weave_imp
_setup(_weave_sys, _weave_imp)
del _weave_sys, _weave_imp
