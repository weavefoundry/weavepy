"""Measure construction, use, and collection of many small LRU caches."""
import functools
import gc
import os
import time
...
if KIND not in ('empty', 'scalar', 'fallback'):
    raise ValueError('unknown LRU population kind: ' + KIND)
...
def bench(n):
    ...
    wrappers = [functools.lru_cache(maxsize=4)(value) for _ in range(n)]
    key = (1,) if KIND == 'fallback' else 1
