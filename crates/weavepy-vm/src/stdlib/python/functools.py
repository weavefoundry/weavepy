...
def _partial_new(cls, func, /, *args, **keywords):
    ...
    if base_cls is partial:
        # WeavePy: `partial` exposes read-only `func`/`args`/`keywords`
        # properties over private slots (CPython's C members).
        self._func = func
        self._args = tot_args
        self._keywords = keywords
    else:
        # `partialmethod` is pure Python in CPython too: plain,
