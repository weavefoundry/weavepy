...
pub unsafe extern "C" fn PyThreadState_SetAsyncExc(
    ...
    exc: *mut crate::object::PyObject,
) -> std::os::raw::c_int {
    // Schedule `exc` (an exception class or instance) to be raised
    // asynchronously in the thread whose `threading.get_ident()` value
    // is `id`. Returns the number of thread states modified: 1 when the
    // ident names a live thread, 0 otherwise (CPython's contract, which
    // test_threading's "nonsensical thread id" probe asserts). Passing
    // NULL clears any pending async exception for that thread.
    let ident = id as u64;
    if !weavepy_vm::vm_singletons::thread_ident_is_live(ident) {
        return 0;
    }
    let pending = if exc.is_null() {
        None
    } else {
        Some(crate::object::clone_object(exc))
    };
    weavepy_vm::gil::set_async_exc(ident, pending);
    1
}
