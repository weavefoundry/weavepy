...
//!
//! Since RFC 0064 this is a *library*: [`cli_main`] is the whole CLI,
...
/// process status).
fn run_multiprocessing_child(raw: &[String]) -> i32 {
...
/// [`cli_main`] with an explicit argv (element 0 is the program name),
/// for the `Py_Main` / `Py_BytesMain` embedding entry points exported
/// by `weavepy-pylib`. The override replaces what `os_args_bridged()`
/// would have read from the process for the duration of the run;
/// undecodable bytes should already be PEP 383-bridged by the caller
/// (`weavepy::vm::os_str_bridged`-style) before they get here.
pub fn cli_main_with_args(args: Vec<String>) -> i32 {
    *ARGV_OVERRIDE.lock().expect("argv override lock") = Some(args);
    cli_main()
}

/// Explicit-argv override for [`cli_main_with_args`]. A process-global
/// (not a thread-local) because `run_on_large_stack` moves execution
/// onto the dedicated `weavepy-main` thread.
static ARGV_OVERRIDE: std::sync::Mutex<Option<Vec<String>>> = std::sync::Mutex::new(None);

/// The CLI's argv: the [`ARGV_OVERRIDE`] if an embedding entry point
/// installed one, else the process argv through the PEP 383 bridge.
/// The override is read non-destructively — `main_dispatch` and
/// `real_main` both consult argv, and they must agree.
fn bridged_args() -> Vec<String> {
    if let Some(args) = ARGV_OVERRIDE.lock().expect("argv override lock").as_ref() {
        return args.clone();
    }
    weavepy::vm::os_args_bridged()
}
...
fn run_on_large_stack(entry: fn() -> i32) -> i32 {
...
fn main_dispatch() -> i32 {
    ...
    let raw: Vec<String> = bridged_args();
