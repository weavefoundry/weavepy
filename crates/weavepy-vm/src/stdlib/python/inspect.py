...
def signature(callable_, *, follow_wrapped=True, globals=None, locals=None, eval_str=False):
    ...
    if not isfunction(obj):
        ...
        if call is not None and (isfunction(call) or ismethod(call)):
            ...
            params = [p for name, p in sig.parameters.items() if name != "self"]
            return Signature(params, return_annotation=sig.return_annotation)
        if call is not None and isbuiltin(call):
            # A native __call__ (e.g. operator.attrgetter's C slot): its
            # Argument-Clinic string already elides `$self`.
            text = getattr(call, "__text_signature__", None)
            if text:
                return _signature_from_text(text)
