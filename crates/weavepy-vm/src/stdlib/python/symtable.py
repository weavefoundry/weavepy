"""Interface to the compiler's internal symbol tables (RFC 0033).

This is CPython 3.13's ``Lib/symtable.py`` wrapper, adapted for WeavePy:

* the native :mod:`_symtable` core returns the raw block tree as a nested
  ``dict`` rather than opaque C objects, so :func:`symtable` wraps each
  node in a small :class:`_RawTable` adapter that exposes the attributes
  the wrapper expects (``type``/``id``/``name``/``lineno``/``nested``/
  ``symbols``/``varnames``/``children``);
* ``SymbolTableType`` is a plain ``str`` subclass instead of
  ``enum.StrEnum`` (WeavePy's ``enum`` has no ``StrEnum`` yet);
* the factory memo is an ordinary ``dict`` rather than a
  ``weakref.WeakValueDictionary``.

Everything else — the ``SymbolTable``/``Function``/``Class``/``Symbol``
API and its semantics — matches CPython.
"""
...
class _RawTable:
    """Attribute view over one raw block dict returned by ``_symtable``."""

    def __init__(self, d):
        self.type = d["type"]
        self.id = d["id"]
        self.name = d["name"]
        self.lineno = d["lineno"]
        self.nested = d["nested"]
...
def symtable(code, filename, compile_type):
    ...
    *filename* is the name of the file with the code
    ...
    """
    top = _RawTable(_symtable.symtable(code, filename, compile_type))
...
class SymbolTableType:
    """Named string constants for symbol-table block kinds.

    CPython exposes these as ``enum.StrEnum`` members; WeavePy's ``enum``
    has no ``StrEnum`` (and ``str`` subclassing is incomplete), so the
    members are plain ``str`` constants. ``get_type()`` returns these, so
    the comparisons users perform (``table.get_type() == "module"`` and
    ``== SymbolTableType.MODULE``) both work.
    """
...
class SymbolTable:
    ...
    def get_type(self):
        ...
        assert False, "unexpected type: {0}".format(self._table.type)
...
class Symbol:
    ...
    def __repr__(self):
        ...
        return '<symbol {0!r}: {1}, {2}>'.format(self.__name, self._scope_str(),
                                                 flags_str)
...
_flags = [('USE', USE)]
_flags.extend((name, value) for name, value in globals().items()
...
def main(args):
    ...
    def print_symbols(table, level=0):
        ...
        if table.get_type() == 'module':
            what = repr(table._filename)
            what = 'from file %s' % what
        else:
            what = repr(table.get_name())
        print('%ssymbol table for %s%s %s:' % (indent, nested,
                                               table.get_type(), what))
        for ident in table.get_identifiers():
            symbol = table.lookup(ident)
            flags = ', '.join(symbol._flags_str()).lower()
            print('    %s%s symbol %r: %s' % (indent,
                                              symbol._scope_str().lower(),
                                              symbol.get_name(), flags))
        print()
