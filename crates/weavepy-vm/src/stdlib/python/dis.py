...
Adapted from CPython 3.13 Lib/dis.py for WeavePy (RFC 0033): the
`_opcode.get_executor` adaptive path is removed since WeavePy does not
expose a quickened code stream. Everything else tracks CPython."""
...
def _get_code_array(co, adaptive):
    # WeavePy emits no adaptive/quickened code stream; always use co_code.
    return co.co_code
...
# NOTE (WeavePy/RFC 0033): CPython sets per-field ``__doc__`` on the
# namedtuple descriptors here. WeavePy's ``collections.namedtuple`` does
# not expose per-field descriptors as class attributes, and these docs are
# purely cosmetic, so they are omitted.
...
class ArgResolver:
    ...
    def get_argval_argrepr(self, op, arg, offset):
        # NOTE (WeavePy/RFC 0033): built-in tuples don't expose
        # ``__getitem__`` as an attribute, so use a subscripting closure.
        get_name = None if self.names is None else (lambda _i: self.names[_i])
        argval = None
        argrepr = ''
        deop = _deoptop(op)
        if arg is not None:
