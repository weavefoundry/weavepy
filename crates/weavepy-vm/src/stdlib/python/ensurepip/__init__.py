...
# WeavePy divergence (RFC 0055 WS2): the bundled wheel is built from
# WeavePy's frozen pip facade (`_minipip`), so the version tracks it
# rather than upstream pip's release number.
_PIP_VERSION = "24.0.0+weavepy"
...
def _get_pip_whl_path_ctx():
    ...
    # WeavePy divergence (RFC 0055 WS2): when this frozen package's
    # reported identity points at a foreign stdlib copy (a vendored
    # CPython `Lib/` on sys.path carries no WeavePy wheel), fall back
    # to the materialized stdlib tree the wheel actually ships in.
    pkg_wheel = Path(os.path.dirname(os.path.abspath(__file__))) / '_bundled' / wheel_name
    if not pkg_wheel.exists():
        stdlib_dir = getattr(sys, '_stdlib_dir', None)
        if stdlib_dir:
            tree_wheel = Path(stdlib_dir) / 'ensurepip' / '_bundled' / wheel_name
            if tree_wheel.exists():
                return nullcontext(tree_wheel)
