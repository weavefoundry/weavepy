...
pub fn build(_cache: &ModuleCache) -> Rc<PyModule> {
    let dict = Rc::new(RefCell::new(DictData::new()));
    {
        ...
        register(&mut d, "raw_unicode_escape_encode", b_raw_unicode_escape_encode);
        register(&mut d, "raw_unicode_escape_decode", b_raw_unicode_escape_decode);
...
fn arg_bytes(args: &[Object], idx: usize, name: &str) -> Result<Vec<u8>, RuntimeError> {
    match args.get(idx) {
        Some(o) => o.as_bytes_view().ok_or_else(|| {
            type_error(format!(
                "{name}() argument {} must be bytes-like",
                idx + 1
            ))
        }),
...
fn decode_utf8(bytes: &[u8], errors: &str) -> Result<String, RuntimeError> {
    match errors {
        "strict" => std::str::from_utf8(bytes)
            .map(str::to_owned)
            .map_err(|e| value_error(format!("'utf-8' codec can't decode byte at position {}", e.valid_up_to()))),
...
fn handle_encode_error(out: &mut Vec<u8>, c: char, errors: &str, encoding: &str) -> Result<(), RuntimeError> {
    match errors {
