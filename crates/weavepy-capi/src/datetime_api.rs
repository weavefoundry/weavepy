...
fn invoke_class_kw(
    ...
    let res = crate::interp::with_interp_mut(|interp| {
        interp.call_object(class_obj.clone(), args, kwargs)
    });
    match res {
...
/// Process-wide guard: the six shells have been registered as
/// inline-instance types (the [`crate::types::INLINE_TYPES`] set is
/// process-global as of RFC 0047 wave 5, so one registration serves every
/// thread). Set once, the first time a datetime class resolves.
static INLINE_DONE: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

/// Cheap gate: is `name` one of the six `datetime` module class names?
pub(crate) fn is_datetime_class_name(name: &str) -> bool {
    matches!(
        name,
...
/// Register the six shells as inline-instance types (idempotent;
/// [`crate::types::maybe_register_inline_type`] skips `tzinfo`, whose
/// `tp_basicsize` is just the object head). All six shells exist before
/// the first call — they are minted together under `DT_INIT_LOCK` — so
/// setting the flag once covers them all.
fn register_inline_once() {
    if INLINE_DONE.swap(true, Ordering::AcqRel) {
        return;
    }
    for slot in [
        &PTR_DATE,
...
pub fn fill_utc_singleton() {
    ...
    let utc =
        crate::interp::with_interp_mut(|interp| match interp.module_cache().get("datetime")? {
            Object::Module(m) => m
                .dict
                .borrow()
                .get(&DictKey(Object::from_static("UTC")))
                .cloned(),
            _ => None,
        })
        .flatten();
...
pub fn maybe_pack_datetime_body(body: *mut PyObject, ty: *mut PyTypeObject, inst: &Rc<PyInstance>) {
    ...
    // Walk `ty` and its `tp_base` chain to find the datetime family it
    // belongs to. A base shell matches on the first step; a *per-subclass*
    // shell (minted for a pure-Python `class Sub(datetime)`) matches at its
    // `tp_base`. `datetime` is checked before `date` at every level because
    // it is the more-derived layout (its own `data[10]` superset). A short
    // guard bounds the walk against a malformed cycle.
    let (date, dt, time, delta) = (
        PTR_DATE.load(Ordering::Relaxed),
        PTR_DATETIME.load(Ordering::Relaxed),
        PTR_TIME.load(Ordering::Relaxed),
        PTR_DELTA.load(Ordering::Relaxed),
    );
    let mut cur = ty;
    let mut guard = 0u32;
    while !cur.is_null() && guard < 16 {
        let c = cur as usize;
        if c == dt {
...
unsafe extern "C" fn date_tp_new(
    ...
    let y = arg_i64(&pos, kw.as_ref(), 0, "year", 1);
    let mo = arg_i64(&pos, kw.as_ref(), 1, "month", 1);
    let d = arg_i64(&pos, kw.as_ref(), 2, "day", 1);
...
unsafe extern "C" fn datetime_tp_new(
    ...
    let mo = arg_i64(&pos, kw.as_ref(), 1, "month", 1);
    let d = arg_i64(&pos, kw.as_ref(), 2, "day", 1);
    let hh = arg_i64(&pos, kw.as_ref(), 3, "hour", 0);
    let mi = arg_i64(&pos, kw.as_ref(), 4, "minute", 0);
    let ss = arg_i64(&pos, kw.as_ref(), 5, "second", 0);
    let us = arg_i64(&pos, kw.as_ref(), 6, "microsecond", 0);
    let fold = arg_i64(&pos, kw.as_ref(), 8, "fold", 0);
    let tz = arg_tzinfo(&pos, kw.as_ref(), 7);
    if std::env::var_os("WEAVEPY_TRACE_NEW").is_some() {
        eprintln!(
...
unsafe extern "C" fn time_tp_new(
    ...
    let (pos, kw) = new_args(args, kwds);
    let hh = arg_i64(&pos, kw.as_ref(), 0, "hour", 0);
    let mi = arg_i64(&pos, kw.as_ref(), 1, "minute", 0);
    let ss = arg_i64(&pos, kw.as_ref(), 2, "second", 0);
    let us = arg_i64(&pos, kw.as_ref(), 3, "microsecond", 0);
    let fold = arg_i64(&pos, kw.as_ref(), 5, "fold", 0);
    let tz = arg_tzinfo(&pos, kw.as_ref(), 4);
        ...
        match &tz {
            ...
            None => {
                ...
                wptr(
                    body,
                    OFF_TIME_TZINFO,
                    crate::object::into_owned(Object::None),
                );
            }
        }
    }
